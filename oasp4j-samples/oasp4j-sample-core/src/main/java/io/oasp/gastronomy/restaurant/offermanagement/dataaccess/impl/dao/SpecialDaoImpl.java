...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriod != null) {
      query
          .where(Alias
              .$(special.getActivePeriod().getEndingHour())
              .eq(activePeriod.getEndingHour())
              .and(
                  Alias
                      .$(special.getActivePeriod().getStartingHour())
                      .eq(activePeriod.getStartingHour())
                      .and(
                          Alias
                              .$(special.getActivePeriod().getEndingDay())
                              .eq(activePeriod.getEndingDay())
                              .and(
